...
#'
#' @description
#' Draws a forest plot from an object of class \code{"cdmeta"}. The plot shows
...
#'   the names of \code{x$data$y} are used when available; otherwise generic
#'   study labels are created.
#' @param order A character string specifying the order of studies in the plot.
#'   Available options are \code{"none"}, \code{"increasing"},
#'   \code{"decreasing"}, \code{"precision"}, and \code{"weight"}.
#' @param level Confidence level used for study-specific and summary intervals.
#'   If \code{NULL}, \code{1 - x$alpha} is used.
#' @param summary_stat Whether Monte Carlo means (\code{"mean"}) or medians
#'   (\code{"median"}) are used as point estimates for summary rows.
#' @param refline Numeric location of the reference line.
#' @param xlab Label for the x-axis.
#' @param main Optional main title.
#' @param atransf Optional function used to transform axis labels and printed
#'   estimates while plotting coordinates remain on the original analysis scale.
#' @param at Optional numeric vector specifying tick-mark locations on the
#'   original analysis scale.
#' @param alim Optional numeric vector of length 2 specifying plotting limits
#'   for the effect-size axis on the original analysis scale.
#' @param xlim Optional numeric vector of length 2 specifying full plotting
#'   limits including the text columns.
#' @param digits Number of digits used for axis labels.
#' @param ci_digits Number of digits used for effect estimates and intervals.
#' @param weight_digits Number of digits used for study weights.
#' @param show_weights Logical indicating whether study weights are shown.
#' @param show_pi Logical indicating whether the prediction interval is shown.
#' @param show_het Logical indicating whether heterogeneity statistics are shown.
#' @param header Logical indicating whether column headers are shown.
#' @param annotate Logical indicating whether estimates and intervals are printed
#'   on the right side of the plot.
#' @param cex Character expansion factor.
#' @param psize Optional point sizes for study-specific estimates. If
#'   \code{NULL}, point sizes are proportional to random-effects inverse-variance
#'   weights.
#' @param pch Plotting symbol for study-specific estimates.
#' @param box_col Border color of study-specific points.
...
#' @param grid Logical indicating whether vertical grid lines are drawn.
#' @param grid_col Color of the grid lines.
#' @param qtype Quantile type used in \code{stats::quantile()}. Default is 8.
#' @param symmetric_shapes Logical indicating whether the summary diamond and
#'   prediction-interval marker are drawn symmetrically around interval midpoints.
#' @param mark_summary_estimate Logical indicating whether the actual summary
#'   point estimate is marked inside the summary diamond.
#' @param mark_prediction_estimate Logical indicating whether the actual
#'   predictive point estimate is marked on the prediction-interval row.
#' @param estimate_mark_col Color of the actual-estimate marks.
#' @param estimate_mark_lwd Line width of the actual-estimate marks.
#' @param mar Plot margins passed to \code{graphics::par()}.
...
#' where \eqn{\alpha = 1 - level}. The summary interval for \eqn{\mu} and the
#' prediction interval are calculated from the Monte Carlo samples in the
#' \code{cdmeta} object when available.
#'
#' If the object was created with \code{transf = exp}, \code{forest()} uses
#' this transformation automatically when \code{atransf = NULL}. Tick marks
#' supplied through \code{at} remain on the original analysis scale.
#'
#' @return Invisibly returns a data frame containing the plotted study-specific
#'   estimates, standard errors, confidence intervals, and weights. Attributes
#'   contain the overall, prediction, and heterogeneity summaries on the original
#'   analysis scale.
#'
#' @examples
#' data(hf_iron)
